...
"use strict";angular.module("core").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise(function($injector,$location){$injector.get("$state").transitionTo("not-found",null,{location:!1})}),$stateProvider.state("home",{url:"/",templateUrl:"modules/core/client/views/home.client.view.html"}).state("add-data",{url:"/:requesterId",templateUrl:"modules/core/client/views/home.client.view.html",controller:"HomeController"}).state("verify-data",{url:"/:type/:id",templateUrl:"modules/core/client/views/home.client.view.html",controller:"HomeController"}).state("not-found",{url:"/not-found",templateUrl:"modules/core/client/views/404.client.view.html",data:{ignoreState:!0}}).state("bad-request",{url:"/bad-request",templateUrl:"modules/core/client/views/400.client.view.html",data:{ignoreState:!0}}).state("forbidden",{url:"/forbidden",templateUrl:"modules/core/client/views/403.client.view.html",data:{ignoreState:!0}})}]);
...
"use strict";angular.module("core").controller("HomeController",["$scope","Authentication","$state","$http","$location","$window","PasswordValidator",function($scope,Authentication,$state,$http,$location,$window,PasswordValidator){$scope.myModel={Url:"http://www.genotible.com",Name:"A platform that's creating a world void of sickle cell anaemia disease, step by step, one genotibility check at a time.",ImageUrl:"modules/core/client/img/brand/couples.png"},$scope.authentication=Authentication,$scope.radioModel="Middle",$scope.isLoading=!1,$scope.isMatching=!1,$scope.religions=["African Tradition","Atheist","Budihism","Christainity","Hinduism","Islam","Judaism","Other"],$scope.genotypes=["AA","AS","AC","SS","SC","I don't know yet"],$scope.matchData={},$scope.feedbackData={},$scope.credentials={},$scope.error={},$scope.matchData.yourReligion=$scope.authentication.user?$scope.authentication.user.religion:$scope.religions[0],$scope.credentials.religion=$scope.religions[0],$scope.matchData.yourGenotype=$scope.authentication.user?$scope.authentication.user.genotype:$scope.genotypes[0],$scope.credentials.genotype=$scope.genotypes[0],$scope.authentication.user&&($scope.matchData.yourEmail=$scope.authentication.user.email),$scope.requesterId=$state.params.requesterId,console.log("previous state params : "+$scope.requesterId),console.log("previous state params : "+$scope.type),console.log("previous state params : "+$scope.id),$scope.records={},$scope.verify=function(){if("verify"===$state.params.type&&null!==$state.params.id){var data={id:$state.params.id};$http.post("/api/auth/verify",data).success(function(response){var msg="Your data was verified successfully!, you can now perfrom Genotibility check.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"information",title:"Information"}),$location.path("/")}).error(function(response){var msg="Data verification failed, please try again";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"error",title:"Error"})})}},$scope.updateRecords=function(){$http.get("/api/records").success(function(response){console.log("fetched records : "+response.verified),$scope.records=response}).error(function(response){console.log(response.message)})},$scope.signup=function(isValid){return $scope.error={},isValid?($scope.isLoading=!0,void $http.post("/api/auth/signup",$scope.credentials).success(function(response){$scope.authentication.user=response,setTimeout(function(){$scope.$apply(function(){$scope.isLoading=!1})},2e3),setTimeout(function(){var msg="Your data was recorded successfully!. Please check your email for verification Link.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"information",title:"Information"}),$scope.requesterId&&$scope.notify({id:$scope.requesterId}),$state.go($state.previous.state.name||"home",$state.previous.params)},2100),$scope.credentials={},$scope.updateRecords()}).error(function(response){$scope.error.dataForm=response.message,setTimeout(function(){$scope.$apply(function(){$scope.isLoading=!1})},2e3),setTimeout(function(){var msg="Oops! something went wrong!, Please confirm you entered a valid data.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"error",title:"Error"})},2100)})):($scope.$broadcast("show-errors-check-validity","userForm"),!1)},$scope.notify=function(data){console.log("sending notify email ......"),$http.post("/api/notify",data).success(function(response){$scope.inviteData=null,$scope.success=response.message,console.log("passed : "+response.message)}).error(function(response){$scope.credentials=null,console.log("failed : "+response.message)})},$scope.sendFeedback=function(isValid){return isValid?($scope.isLoading=!0,console.log("feedbackData email : "+$scope.feedbackData.yourEmail),console.log("feedbackData message : "+$scope.feedbackData.message),void $http.post("/api/feedback/send",$scope.feedbackData).success(function(response){setTimeout(function(){$scope.$apply(function(){$scope.isLoading=!1})},2e3),setTimeout(function(){console.log("response is : "+response);var msg="Thanks for the feedback, it was sent successfully.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"information",title:"Information"})},2100),$scope.feedbackData={}}).error(function(response,status){setTimeout(function(){$scope.$apply(function(){$scope.isLoading=!1})},2e3),setTimeout(function(){var msg="Error sending feedback.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"error",title:"Error"})},2100),response&&console.log("Error message : "+response.message)})):($scope.$broadcast("show-errors-check-validity","feedbackForm"),!1)},$scope.runMatch=function(isValid){return console.log("called runMatch"),console.log(" runMatch is valid : "+isValid),$scope.error={},isValid?($scope.isMatching=!0,void $http.post("/api/match",$scope.matchData).success(function(response){console.log("response is : "+response),setTimeout(function(){$scope.$apply(function(){$scope.isMatching=!1})},2e3),setTimeout(function(){switch(response){case 1:$.Zebra_Dialog("You are genotible, But of different faith! \nThe decision is yours ...");break;case 2:$.Zebra_Dialog("You are not genotible! \n  We advice you to move on");break;case 3:$.Zebra_Dialog("You are genotible! \nWe advice you to make a move...");break;default:var msg="Error processing match, please check the data provided.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"error",title:"Error"})}},2100),$scope.matchData={},$scope.updateRecords()}).error(function(response,status){response?(console.log("Error message : "+response.message),console.log("Error status : "+status),600===status?(console.log("send email"),$scope.sendInvite({email:$scope.matchData.spouseEmail,requesterEmail:$scope.matchData.yourEmail}),$scope.matchData={}):700===status?(setTimeout(function(){$scope.$apply(function(){$scope.isMatching=!1})},2e3),setTimeout(function(){var msg="Please add info for `"+$scope.matchData.yourEmail+"` in the `Add Data section` before performing a genotible match.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"information",title:"Information"})},2100)):800===status?(setTimeout(function(){$scope.$apply(function(){$scope.isMatching=!1})},2e3),setTimeout(function(){var msg="Please you need to verify your data, before doing genotibility check. Check your email `"+$scope.matchData.yourEmail+"` for genotible verification mail.";$.Zebra_Dialog("<strong>"+msg+"</strong>",{type:"information",title:"Information"})},2100)):(setTimeout(function(){$scope.$apply(function(){$scope.isMatching=!1})},2e3),setTimeout(function(){$.Zebra_Dialog("<strong>Oops! Data loading error, please try again.</strong>",{type:"error",title:"Error"})},2100))):($scope.error.checkForm=response.message,setTimeout(function(){$scope.$apply(function(){$scope.isMatching=!1})},2e3),setTimeout(function(){$.Zebra_Dialog("<strong>Error processing match, please check the data provided.</strong>",{type:"error",title:"Error"})},2100))})):($scope.$broadcast("show-errors-check-validity","matchForm"),!1)},$scope.sendInvite=function(data){console.log("sending email ......"),$http.post("/api/invite",data).success(function(response){$scope.inviteData=null,$scope.success=response.message,setTimeout(function(){$scope.$apply(function(){$scope.isMatching=!1})},2e3),setTimeout(function(){$.Zebra_Dialog("<strong>"+response.message+"</strong>",{type:"information",title:"Information"})},2100)}).error(function(response){$scope.credentials=null,$scope.error=response.message,$scope.isMatching=!1,$.Zebra_Dialog("<strong>"+response.message+"</strong>",{type:"error",title:"Error"})})},$scope.updateRecords(),$scope.$apply(function(){$scope.verify()})}]);
...
"use strict";angular.module("users").controller("AuthenticationController",["$scope","$state","$http","$location","$window","Authentication","PasswordValidator",function($scope,$state,$http,$location,$window,Authentication,PasswordValidator){$scope.authentication=Authentication,$scope.popoverMsg=PasswordValidator.getPopoverMsg(),$scope.error=$location.search().err,$scope.requesterId=$state.params.requesterId,console.log("previous state params : "+$scope.requesterId),$scope.signup=function(isValid){return $scope.error=null,isValid?void $http.post("/api/auth/signup",$scope.credentials).success(function(response){$scope.authentication.user=response,alert("Your data was recorded successfully!."),$scope.requesterId&&$scope.notify({id:$scope.requesterId}),$state.go($state.previous.state.name||"home",$state.previous.params)}).error(function(response){$scope.error=response.message,alert("Opps! something went wrong!, Please try again.")}):($scope.$broadcast("show-errors-check-validity","userForm"),!1)},$scope.notify=function(data){console.log("sending notify email ......"),$http.post("/api/notify",data).success(function(response){$scope.inviteData=null,$scope.success=response.message,console.log("passed : "+response.message)}).error(function(response){$scope.credentials=null,$scope.error=response.message,console.log("failed : "+response.message)})},$scope.signin=function(isValid){return $scope.error=null,isValid?void $http.post("/api/auth/signin",$scope.credentials).success(function(response){$scope.authentication.user=response,$state.go($state.previous.state.name||"home",$state.previous.params)}).error(function(response){$scope.error=response.message}):($scope.$broadcast("show-errors-check-validity","userForm"),!1)},$scope.callOauthProvider=function(url){$state.previous&&$state.previous.href&&(url+="?redirect_to="+encodeURIComponent($state.previous.href)),$window.location.href=url}}]);
...
"use strict";angular.module("users").controller("EditProfileController",["$scope","$http","$location","Users","Authentication",function($scope,$http,$location,Users,Authentication){$scope.user=Authentication.user,$scope.religions=["Islam","Christainity"],$scope.genotypes=["AA","AS","AC","SS"],$scope.updateUserProfile=function(isValid){if($scope.success=$scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","userForm"),!1;var user=new Users($scope.user);user.$update(function(response){$scope.$broadcast("show-errors-reset","userForm"),$scope.success=!0,Authentication.user=response},function(response){$scope.error=response.data.message})}}]);
